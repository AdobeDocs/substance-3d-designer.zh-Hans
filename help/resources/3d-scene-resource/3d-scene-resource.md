...
将3D场景链接到包是从该场景几何形状[生成网格图](../../bakers/bakers.md)的唯一方法。 您可以执行以下步骤以开始使用：

* 在包上单击&#x200B;*人民币*，然后在上下文菜单中选择<b>链接> 3D网格</b>选项
* 选择任何受支持的3D场景文件
* 如果显示<b>链接为Udim网格</b>对话框提示，请单击&#x200B;*否*，除非您要烘焙UV磁贴
* 在[资源管理器](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/the-explorer-129368147.html)中加载资源后，单击资源上的&#x200B;*人民币*，然后在上下文菜单中选择<b>烘焙模型信息</b>选项
...
当链接某个网格资源且应用程序检测到该资源具有超出0-1范围的UV时，系统会询问您是否应该将该网格视为UDIM网格（也称为UV磁贴）。 此设置以后可以更改，除非您确定您使用的是UV磁贴，否则应回答<b>否</b>。

如果UV拼贴行为处于活动状态，则烘焙行为有所不同，并将为每个检测到的UV拼贴烘焙纹理。

## 资源/场景与状态

该应用程序将您在3D视图中看到的内容分成两个不同的文件。 实际的3D模型或网格是在浏览器中可见的资源。 光线、相机和其他设置的设置称为“<b>状态</b>”。 可以将状态保存到外部.sbsscn文件，以便以后再次加载。 .sbsscn文件不是资源，它们是只能通过[3D视图中的“场景”菜单加载的附加配置文件。](../../interface/3d-view/3d-view.md)
