...
* 注释适合在框架描述所允许范围之外添加其他文本。 您可以在每个节点中添加少量的文本，主要用于提供一些细节信息。 评论的缩放效果不佳，而且无法从遥远的缩放级别进行阅读。
* 导航图钉允许您使用F2快捷键循环浏览图形的特定区域。 这对于非常大的图表非常有用，在这些图表中，用户通常需要在彼此距离非常远的两个区域之间跳转。

### 输入和输出放置

输入和输出应放在图形的极端：所有输出在右侧，所有输入在左侧，每个输入垂直对齐。 这样可以更容易地找到和识别它们。

![输入和输出放置](../../assets/inout.gif "输入和输出放置")

上例是一个极端情况：帧并非始终必需或可能，但应清楚的是，入点和出点的垂直对齐远比随机随机随机随机随机放置清晰。

### 链接重路由

在大型、超长图表中，有时链接是在超大范围内进行的。 这会导致交叉穿过图表的链接线变得混乱，且无需太多控制。 快捷键“Alt + Shift拖动”允许您重新组织这些链接，通过细分链接并在中间添加一个额外的手柄来在不同的路径上重新路由它们。 建议在可行的情况下使用它。

![链接重路由](../../assets/linkjreroute.gif "链接重路由")

### 标签、标识符和用法

任何用于共享或发布的图表都应适当谨慎地添加到其他元数据中，以提高用户友好性。 以下几点很重要：

默认建议的标签永远不够，需要花费时间和精力将自定义标签添加到显示的参数以及输入和输出。

![标识符和标签](../../assets/output-label.png "标识符和标签")

尽量不要使用标识符和标签差异太大：如果标识符用于其他地方（在多个函数中），则很难找到与哪个变量相关的UI属性。

![标识符清晰度](../../assets/labelvsidentifier.png "标识符清晰度")

尝试使标签与您在帧（帧标签）和注释中使用的术语匹配。 这样可以更轻松地找出图表的哪个部分与显示的参数有关
