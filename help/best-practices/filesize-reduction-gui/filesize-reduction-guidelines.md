...
+++图形中的使用情况
将“位图”节点设置为哪种模式也会影响Designer压缩文件的方式，在图形中使用灰度模式文件作为彩色位图会产生更大的文件。 确保正确设置这些项！
...
下表显示了所有设置彼此的影响。 所用的位图是生成的杂色的4096x4096图像，从Photoshop导出为24位TGA或JPG，品质为8。 TGA也导出为灰度和RGBA模式。

图表仅放置连接到单个输出的单个位图节点。 位图模式根据源文件模式进行设置。
...
> **Jpeg位图格式**
> 
> 请务必注意，需要高精度的特殊地图（如法线地图、矢量地图等）可能不会设置为Jpeg压缩，因为这会导致更明显的伪影！
