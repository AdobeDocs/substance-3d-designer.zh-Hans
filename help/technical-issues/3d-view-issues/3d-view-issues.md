...
breadcrumb-title: ''
description: 解决Substance 3D Designer中的3D 视图问题，包括渲染、显示和性能问题。
...
在一个会话中以详细卷为特征的3D对象在下一个会话中变为平坦，但是图形没有改变，并且高度图承载相同数据。

**![（刻度）](3d-view-issues.resources/check.svg)建议的步骤**

根据高度图的3D对象的变形效果使用一种称为&#x200B;**曲面细分位移**&#x200B;的技术来执行。 这项技术包括两个步骤：

1. **曲面细分**：对象几何被&#x200B;*细分*&#x200B;为顶点，从而产生&#x200B;*更密集的几何*&#x200B;以支持更精细的卷细节
2. **位移**：顶点沿其&#x200B;*正常矢量*&#x200B;被&#x200B;*移动* — 即位移。 法向量沿多边形所面对的方向，并具有模数（即长度）为1的量

位移&#x200B;*方向*&#x200B;是已知的：法向矢量的方向。\
移动顶点所依据的位移&#x200B;*距离*&#x200B;的计算方法如下： `Distance = Height scale * Height map`。 由于高度图在图形中&#x200B;*未更改*，因此会保留&#x200B;**Height比例**。

默认Height比例值为&#x200B;**1.0**，根据3D 视图中显示的网格及其应用的高度图，这可能会产生&#x200B;*不明显的*&#x200B;位移效果。
...
>[!TIP]
>
> 使用此方法，您可以设置自定义Height比例值&#x200B;*每个图形*，这样您就可以调整它以匹配该图形的特定材料。

## 3D视图完全为黑色

**![（错误）](3d-view-issues.resources/error.svg)问题**

在版本15.0.0及更高版本中，3D视图的视口为纯黑色。 我看到一些文本叠加（例如，采样和渲染时间），但3D 场景不可见。
