...
| *参数* | *描述* |
| --- | --- |
| **方法** | 控制烘焙纹理将与Substance包一起存储的方式。可能的值：<ul data-preserve-html="true"><li data-preserve-html="true"><strong>嵌入</strong> ：烘焙纹理存储在具有特定命名的Substance包旁边的子文件夹中。</li><li data-preserve-html="true"><strong>已链接</strong>（默认） ：烘焙的纹理存储在定义的文件夹中，然后引用到Substance包中。</li></ul> |
| **文件夹** | 保存时烘焙的纹理的位置。 单击三点式按钮打开一个文件对话框并选择导出文件夹。右侧将显示一个复选标记，指示文件夹是否实际存在。 |
| **名称** | 纹理的命名约定。 单击三点式按钮可打开下拉列表，并插入其他占位符(bakename、custom、材料、网格)。 |
| **示例** | 模拟文件名以测试命名约定。 |
| **将资源放入网格特定的文件夹** | 如果启用，烘焙的纹理将保存在名为“网格文件”的文件夹中。 |
...
![高清晰度网格](bakers.resources/sd-high.png "高清晰度网格")
...
在&#x200B;**Baker渲染列表**&#x200B;中，您可以选择要生成的烘焙纹理。 默认情况下，该列表为空。

* **添加新Baker：**&#x200B;单击“添加Baker”按钮。
* **删除Baker：**&#x200B;在列表中选择Baker，然后单击“删除Baker”按钮。
* **将Baker移动到顶部：**&#x200B;在列表中选择Baker，然后单击“拉至顶部”按钮。
* **下移一个Baker：**&#x200B;在列表中选择Baker，然后单击“下移”按钮。

每个Baker在默认情况下都继承默认值（请参见上文）。 例如，可以通过单击Baker行上的单元格来覆盖大小（分辨率）。 这适用于行中的其他设置。

单击列表中的Baker时，“Baker参数”视图将以其特定参数更新。

要了解有关特定参数的详细信息，请参阅： [Baker设置](https://experienceleague.adobe.com/zh-hans/docs/substance-3d/bakers/bakers-settings/bakers-settings)。

![Baker渲染列表](bakers.resources/sd-baker-list.png "Baker渲染列表")
