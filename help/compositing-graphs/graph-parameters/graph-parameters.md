...
|                      |                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|:---------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <b>输出大小</b> | 此参数允许您在图形中选择图像的&#x200B;*基分辨率*。  使用 <div><img data-preserve-html="true" height="22" src="graph-parameters.resources/props-output-size-lock.jpg"/></div> 锁定按钮，使高度值和宽度值匹配，并在调整大小时保持图像正方形。<br><br>*默认： (0,0) -相对于父代* [了解详情](../../compositing-graphs/output-size/output-size.md) |
...
| **类型** | <b>类型</b>用于定义[图形](../../compositing-graphs/substance-compositing-graphs.md)的预期用途。 它主要用于[“发送”互操作性功能](../../interface/the-explorer-window/send-to-interoperability/send-to-interoperability.md)。 |
| **材质模型** | 设置图形的材质模型可确保3D 视图中使用适当的着色器（如果&#x200B;*与模型*&#x200B;匹配的着色器可用）。<br>例如， 查看3D 视图中具有`OpenPBR v1.1`材质模式的图形将为目标材料选择中的`OpenPBR Surface`着色器。<br><br>如果未找到匹配的着色器，或者图形的模型设置为`Undefined`，则在3D 视图中用于目标材料的着色器为&#x200B;*未更改*。 |
| **物理尺寸** | 此值指定&#x200B;*物理世界*&#x200B;中纹理的尺寸，格式为X（长度）、Y（宽度）和Z(Height)。 因此，它与在图形中产生的材料有内在联系。 例如，可以使用物理尺寸在<b>2D 视图</b>和<b>3D 视图</b>中按正确比例显示纹理。<br><br>*提示：*&#x200B;可以使用$physicalsize [内置变量](../../function-graphs/variables/system-variables/system-variables.md)，将Substance图形的物理尺寸作为应用于该图形中任何Float的Substance函数3D 视图中的图形3值进行检索。<br><br>*注意：*&#x200B;在&#x200B;**&#x200B;**&#x200B;中，**Z**&#x200B;值当前&#x200B;*未考虑在内*。 因此，应该使用设置为&#x200B;**高阶**&#x200B;使用的&#x200B;**输出**&#x200B;材料设置Height比例&#x200B;**值，或者直接在**&#x200B;材料属性&#x200B;**中设置。<br><br>*默认值： (0,0，0)*** |
| **图标** | 此区域允许您定义&#x200B;*图标*，<b>库</b>将使用此图标以<b>SBS</b>和<b>SBSAR</b>形式显示此图形的条目。 此图标还用于其他情况，如[Substance 3D Painter](https://www.adobe.com/cn/products/substance3d-painter.html)的<b>工具架</b>。 该区域提供以下选项：<ul data-preserve-html="true"> <li data-preserve-html="true"><b>浏览</b>：允许您浏览系统文件以查找<i>应用作图标的现有映像</i></li> <li data-preserve-html="true"><b>生成</b>：这将使用<b>PBR 渲染</b>节点的<i>内置预设</i>生成图标</li> <li data-preserve-html="true"><b>粘贴</b>：允许您将当前位于<i>剪贴板</i>中的图像数据粘贴为图标</li> <li data-preserve-html="true"><b>移除</b>：此选项<i>移除</i>现有图标，并将图标槽<i>留空</i></li> </ul>*注意：* **生成**&#x200B;选项使用&#x200B;**物理尺寸**&#x200B;来确定&#x200B;**PBR 渲染**&#x200B;的&#x200B;**Height比例**&#x200B;的位移效果。 如果图形中存在设置为&#x200B;**物理大小**&#x200B;用法的&#x200B;**输出**&#x200B;节点，则使用此输出。 如果不存在此类输出，则使用图形&#x200B;**Attributes**&#x200B;中的值&#x200B;*代替*。 如果属性的值为(0,0，0)，则使用0.1的&#x200B;*预设值*。<br><br>*注意：*&#x200B;未定义&#x200B;*图标*&#x200B;时，将改用图形的&#x200B;*第一个图像输出*。<br><br>*默认：空* |
...
| **模板数据** | 将图形用作模板时，此属性设置[模板的类别和副标题](../../compositing-graphs/creating-compositing-gra/creating-a-substance-compositing-graph.md)。 它们由此分隔： &lt;category>；&lt;subtitle> <br><br>*默认值：空* |
...
打开“输入参数”部分顶部的汉堡包菜单，然后转到“覆盖基本参数”子菜单，以选择要为其设置任意默认值的基本参数。

所选参数的编辑器将出现在输入参数列表的顶部。 然后，您可以根据需要调整它们的值和[继承方法](../../compositing-graphs/inheritance-compositing/inheritance-in-substance-compositing-graphs.md)。
...
在此部分中，列出了所有图形的[输入](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/input/input.md)节点。
...
在此部分中，所有图形的[输出](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/output/output.md)节点。
