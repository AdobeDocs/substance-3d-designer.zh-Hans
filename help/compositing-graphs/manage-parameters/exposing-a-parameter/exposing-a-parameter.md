...
![简化了公开参数](exposing-a-parameter.resources/exposing-a-parameter-01.png "简化了公开参数")
...
![单公开方法演练](exposing-a-parameter.resources/exposing-a-parameter-02.gif "单公开方法演练"){width="512px"}

### 单曝光法

1. 在“[属性](../../../interface/properties/properties.md)”面板的“特定参数”选项卡下找到要公开的参数
1. 单击![](exposing-a-parameter.resources/exposing-a-parameter-03.png)下拉选项按钮
1. 从下拉列表第一个选项中选择![](exposing-a-parameter.resources/exposing-a-parameter-04.png) <b>公开为新图形输入</b>。
1. 出现<b>公开参数</b>对话框，根据需要设置任何参数属性。

   建议至少更改<b>标识符</b>和<b>标签</b>
1. 按<b>确定</b>以确认
1. 参数名称变为&#x200B;*蓝色*，变为![](exposing-a-parameter.resources/exposing-a-parameter-05.png)\
...
![批量公开方法演练](exposing-a-parameter.resources/exposing-a-parameter-06.gif "批量公开方法演练"){width="512px"}

### 批量公开方法

公开一个参数时，此方法将比上一个方法慢一些。 公开多个参数时，速度要快得多。

1. 查找<b>特定参数</b>选项卡右上角的![](exposing-a-parameter.resources/exposing-a-parameter-07.png) <b>多公开</b>按钮，而不是查找单个参数
...
1. 单击<b>确定</b>以确认并公开所有选定的参数。 参数名称现在显示&#x200B;*蓝色*&#x200B;以确认参数已公开，同时显示![](exposing-a-parameter.resources/exposing-a-parameter-05.png) <b>编辑函数</b>按钮。
...
1. 单击已公开参数旁边的下拉选项按钮![](exposing-a-parameter.resources/exposing-a-parameter-03.png)
1. 选择“![](exposing-a-parameter.resources/exposing-a-parameter-04.png)<b>”“编辑公开的图形输入”</b>。 该操作会将您直接带到图表属性中的相关条目
1. 双击图形的空白区域以转到图形属性，然后在<b>输入参数</b>列表中查找该参数
1. 单击<b>资源管理器</b>中的图形，然后在<b>输入参数</b>列表中查找该参数

![输入参数](exposing-a-parameter.resources/exposing-a-parameter-08.png "输入参数"){width="512px"}
...
![整数参数的下拉列表编辑器](exposing-a-parameter.resources/exposing-a-parameter-09.gif "整数参数的下拉列表编辑器"){width="512px"}
...
![对输入参数重新排序](exposing-a-parameter.resources/exposing-a-parameter-10.gif "对输入参数重新排序"){width="512px"}

#### 重新排序

通过拖放列表中输入参数名称左侧的深色条带化手柄，可以轻松对参数重新排序。 请记住，分组参数会影响顺序。

![预览输入参数](exposing-a-parameter.resources/exposing-a-parameter-11.gif "预览输入参数"){width="512px"}
...
![复制和粘贴输入参数](exposing-a-parameter.resources/exposing-a-parameter-12.gif "复制和粘贴输入参数"){width="512px"}

### 复制粘贴参数

可以在图形之间复制粘贴参数。

可以使用复制按钮![](exposing-a-parameter.resources/exposing-a-parameter-13.png)复制单个参数。 可以通过参数菜单![](exposing-a-parameter.resources/exposing-a-parameter-07.png)复制多个参数。 选择“复制输入”以复制所有输入。

在“参数”菜单![](exposing-a-parameter.resources/exposing-a-parameter-07.png)中选择“粘贴输入”![](exposing-a-parameter.resources/exposing-a-parameter-14.png)以粘贴一个或多个参数。
...
![节点参数出错](exposing-a-parameter.resources/exposing-a-parameter-15.gif "节点参数出错"){width="512px"}

### 跟踪节点上的损坏参数

您可以通过“节点查找器工具”![](exposing-a-parameter.resources/exposing-a-parameter-16.png)（位于“图形视图”顶栏中）跟踪哪个节点使用了哪个参数。 单击它可让您使用特定参数查找节点。

如果节点遇到实际问题，则会在节点的左上角显示警告标记![](exposing-a-parameter.resources/exposing-a-parameter-17.png)。 将鼠标悬停在徽章上会显示工具提示，其中包含更多信息。

要重置并移除问题，对于要修复或重置的参数，单击“编辑函数”按钮旁边的下拉按钮![](exposing-a-parameter.resources/exposing-a-parameter-03.png)并选择![](exposing-a-parameter.resources/exposing-a-parameter-18.png) <b>重置。 </b>这将使参数返回到其以前的非公开状态，蓝色名称将再次变为灰色以反映此情况。

![清除未使用的输入参数](exposing-a-parameter.resources/exposing-a-parameter-19.gif "清除未使用的输入参数"){width="512px"}

### 清除未使用的输入参数

如果您丢失了输入参数的跟踪信息并且不再知道使用了哪些参数，可以使用小工具清理它们。 单击“输入参数”菜单按钮![](exposing-a-parameter.resources/exposing-a-parameter-07.png)，然后选择<b>清理输入。</b>

此时会出现一个新对话框，其中列出了所有未使用的参数。 选中或取消选中要删除或保留的任何参数，然后单击“确定”。 如果未出现对话框，则当前没有要清理的未使用参数。

![正在删除参数](exposing-a-parameter.resources/exposing-a-parameter-20.gif "正在删除参数"){width="512px"}

### 删除参数

要实际移除正在使用的参数，需要两个不同的步骤。

1. 在具有已公开参数的节点上，单击“函数公开”按钮右侧的绿色下拉箭头：![](exposing-a-parameter.resources/exposing-a-parameter-21.png)。 然后选择“重置为默认值”。 这样将删除在该节点上对该参数的使用。 对使用相同参数的任何其他节点重复此步骤。 “重置为默认值”也会将参数小部件的范围重置为其&#x200B;*软范围*。
