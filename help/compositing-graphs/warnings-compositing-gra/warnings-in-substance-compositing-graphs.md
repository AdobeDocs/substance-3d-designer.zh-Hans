...
>[!NOTE]
>
> 通过[新建图形](../creating-compositing-gra/creating-a-substance-compositing-graph.md)对话框提供的图形模板具有可以使用的预设输出节点。

![修复“未定义输出节点”警告](../../assets/warnings-comp-output.gif "修复“未定义输出节点”警告"){width="512px"}

### ![（错误）](../../assets/error.svg) *[x]*&#x200B;参数的函数有一些警告

应用于指定图形的指定参数的[函数Node](../../function-graphs/function-graphs.md)至少有一个警告。\
节点参数在节点标签后面的方括号之间指定，位于模板Node[Parameter]之后。

E.g. 统一颜色[Output Color]，像素处理器[Per Pixel Function]
...
然后，评估图形视图左下角列出的警告并解决问题。 您可以参阅[函数图形中的警告](../../function-graphs/warnings-function-graphs/warnings-in-function-graphs.md)页，对函数图形中报告的警告进行故障诊断。
...
否则，请删除该实例化并将其替换为引用有效包的实例化。 或者，您可以重新创建实例化引用的包和图形，然后通过在[资源管理器](../../interface/the-explorer-window/the-explorer-window.md)面板中单击宿主包上的人民币并在上下文菜单中选择&#x200B;**重新加载**&#x200B;选项来重新加载宿主包。
...
否则，请在资源管理器中单击资源项上的RMB，然后在上下文菜单中选择&#x200B;**重定位...**&#x200B;选项以设置该资源的新有效目标文件。
