...
它是Substance 3D Designer中最核心且最有用的节点之一，通常用于重新映射和调整图表中的值，因为它为更改值提供了最精确和准确的界面。
...
|  |  |
| --- | --- |
| <b>低处的级别</b> *浮动/浮动4* | 定义输入图像的低光级别。 重新映射输入的Low值，使其变为全黑色。 |
| <b>高处的级别</b> *浮动/浮动4* | 定义输入图像的高光级别。  将输入的High值重新映射为全白色。 |
| <b>中间级别</b> *浮动/浮动4* | 定义输入图像的中间色调级别。  将输入Mid值重新映射为中间灰色。 |
| <b>水平输出低</b> *浮动/浮动4* | 定义输出图像的低光级别。  固定输出Black值以设置限制。 |
| <b>水平超高</b> *浮动/浮动4* | 定义输出图像的高光级别。  钳制输出白色值以设置限制。 |
