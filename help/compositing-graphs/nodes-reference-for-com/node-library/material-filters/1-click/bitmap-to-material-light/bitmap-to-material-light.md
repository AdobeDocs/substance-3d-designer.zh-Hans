...
* **Specular**
  * **Diffuse影响**： *0.0 - 1.0*&#x200B;设置扩散对Specular的影响。 还影响光泽度和粗糙度输出。
  ...
  * **Specular影响**： *0.0 - 1.0*&#x200B;确定可选金属输入对Specular映射的影响。
* **光泽度**
  * **在**&#x200B;中的光泽度级别： *0.0 - 1.0*&#x200B;设置用于光泽度解释的输入级别。
  * **输出光泽度色阶**： *0.0 - 1.0*&#x200B;修改光泽度输出色阶。
  * **光泽度影响**： *0.0 - 1.0*&#x200B;确定可选金属输入对光泽度映射的影响。
* **粗糙度**
  * **在**&#x200B;中的粗糙度级别： *0.0 - 1.0*&#x200B;设置用于粗糙度解释的输入级别。
  * **输出粗糙度色阶**： *0.0 - 1.0*&#x200B;修改粗糙度输出色阶。
  * **金属粗糙度影响**： *0.0 - 1.0*&#x200B;确定可选金属输入对光泽度映射的影响。
* **环境遮蔽**
  * **Diffuse中的Ambient occlusion**： *0.0 - 1.0*&#x200B;将生成的AO中的混合输出为Diffuse。
  * **Ambient occlusion跨页**： *0.0 - 1.0*&#x200B;设置生成AO跨页的距离。
  * **Ambient occlusion光距离**： *0.0 - 1.0*&#x200B;设置AO“深度”解释。 当跨距较大时，影响较小。
  * **Ambient occlusion的光照角度**： *0.0 - 1.0*&#x200B;设置虚假的光照AO强制转换角度。 如果设置为相反的角度，可用于补偿Diffuse中已有的任何方向AO。
  * **Ambient occlusion级别**： *0.0 - 1.0*&#x200B;修改AO输出级别。
