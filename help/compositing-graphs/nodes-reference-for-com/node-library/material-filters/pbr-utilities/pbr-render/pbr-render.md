...
* **渲染设置**
  ...
  * **Ambient occlusion强度**： *0.0 - 1.0*\
    设置ambient occlusion阴影的强度。
* **材料调整**
  * **重新计算法线**： *False/True*\
    将根据位移强度从高度图中重新计算法线。
  * **普通格式**： *DirectX，OpenGL*\
  ...
  * **电介质F0输入**： *常量值，Specular level输入*\
    设置什么驱动F0值。 Specular level输入表示它将由输入图驱动。
  * **电介质F0**： *0.0 - 0.08*\
...
* **透明外套**
  ...
  * **透明外套重量**： *0.0 - 1.0*\
    设置透明涂层图层的强度或强度。
  * **清除Coat specular level**： *0.0 - 1.0*\
    设置透明涂层的粗糙度。
  * **从基底图层继承Normal**： *False/True*&#x200B;如果透明皮忽略或使用来自基础材质的法线，则设置。
* **Emissive**
  * **启用Emissive光照** *True/False*&#x200B;切换emissive光照的扩散作用。
  * **Emissive强度**： *0.0 - 10.0*\
    设置emissive映射的全局乘数。
* **次表面散射**
  * **启用次表面散射** *True/False*\
