...
breadcrumb-title: ''
description: 使用“裁剪”节点将材料输出裁剪到特定区域，以便处理扫描的材料和纹理。
...
此功能在许多方面都很有用，因为使用原子节点执行“裁剪”操作并不那么简单。 尤其是在变换非正方形图像时，这个节点就派上用场了。 在这种情况下，请确保正确设置输入分辨率。
...
| <b>偏移</b> <i>0.0 - 1.0</i> | 移动或平移结果。 可以通过与画布直接交互来修改描摹结果。 |
