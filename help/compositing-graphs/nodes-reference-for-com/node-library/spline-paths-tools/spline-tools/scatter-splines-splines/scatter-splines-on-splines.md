...
|  |  |
| --- | --- |
| <b>启动Height模式</b> *整数* | 一种计算离散样条起始Height的方法。   手动：为所有散布的样条设置相同的绝对值。   从父样条（+自定样条）使用父样条的Height，然后使用<b>自定样条起始Heightmult.</b>添加自定样条的Height。 参数。   从自定样条：按原样使用自定样条的Height。   *注意：*&#x200B;将<b>样条类型</b>设置为“自定义样条”，并连接<b>自定义样条</b>输入以使用自定义样条的Height。 |
...
| <b>结束Height模式</b> *整数* | 计算离散样条结束Height的方法。   手动：为所有散布的样条设置相同的绝对值。   从父样条（+自定样条）使用父样条的Height，然后使用<b>自定样条端点Height多数</b>添加自定样条的Height。 参数。   从自定样条：按原样使用自定样条的Height。     *注意：*&#x200B;将<b>样条类型</b>设置为自定义样条，并连接<b>自定义样条</b>输入以使用自定义样条的Height。 |
