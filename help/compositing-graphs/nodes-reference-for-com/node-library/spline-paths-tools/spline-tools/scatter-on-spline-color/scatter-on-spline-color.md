...
|  |  |
|:---|:---|
| <b>样条输入</b> <i>整数</i> | 选择哪些样条应该用于散布模式的方法： <br>**&#x200B;所有样条&#x200B;*：使用输入列表中的所有样条；<br>* *单样条*：仅使用输入列表中的指定样条；<br>* *样条范围*：仅使用输入列表中的指定范围内的样条。 |
| <b>样条索引</b> <i>整数</i>（在“样条输入”设置为“单样条”时可用） | 用于散射模式的样条的列表索引。 |
| <b>样条范围</b> <i>整数2</i>（在“样条输入”设置为“样条范围”时可用） | 列表索引的范围，其中包括应该用于散射模式的样条。 |
| <b>散点模式</b> <i>整数</i> | 沿样条散布图案的方法，该方法影响每个样条上的图案量：<br>*形状量：散布指定数量的均匀间隔图案；<br>*形状间距：自动调整图案数量以适合指定的均匀间距。<br>在这两种情况下，第一个和最后一个模式分别精确落在每个样条的起始处和结束处。 |
...
| <b>模式输入分布</b> <i>整数</i> （当“Pattern”设置为“Pattern Input”时可用） | 用于选择哪些输入模式应该散布在给定样条上的方法：<br>*— 随机*：随机选择模式；<br>*— 沿样条*：模式索引沿样条逐渐增加；<br>*— 模式索引*：沿每个样条循环输入模式的索引；<br>*— 样条索引*：在输入样条列表中从一个样条到下一个样条循环输入模式的索引。 |
...
| <b>重复</b> |  |
| <b>分发模式</b> <i>整数</i> | 放置复制图案的方法：<br>*— 线性*：复制图案沿样条法向与图案原始位置均匀隔开；<br>*— 圆形*：复制图案沿图案原始位置处以样条为中心的虚圆排列。 |
...
| <b>大小</b> |  |
| <b>大小模式</b> <i>整数</i> | 设置散布图案大小的方法：<br>*— 法线*：使用全局“缩放”参数统一控制大小；<br>*— 使用样条的Thickness*：大小由样条的Thickness驱动。 |
| <b>Thickness影响</b> <i>整数</i>（在“大小模式”设置为“使用来自样条的Thickness”时可用） | 指定图案比例的轴应由样条的Thickness驱动： <br>* X &amp; Y：Thickness与X和Y轴的大小相乘；<br>* X：Thickness仅与X轴的大小相乘；<br>* Y：Thickness仅与Y轴的大小相乘。<br>如果不进行相乘，图案的原始比例将是图像的整个范围。<br>这意味着在“X”模式下，Y轴的大小是图像的完整范围，需要使用Size参数进行调整。 使用“Y”模式时，这同样适用于X轴中的大小。 |
...
| <b>随机颜色中心</b> <i>浮动</i> | 将偏移应用于随机颜色中应用的随机偏移的范围<br>值为–1表示所有随机值都较高，值为1表示所有随机值都较低。 |
...
| <b>样条Thickness乘数</b> <i>浮动</i> | 每个图案的Alpha与其所在位置的样条Thickness相乘的强度。<br>注意：输出颜色是所有颜色乘数的加权结果。 |
| <b>形状缩放乘数</b> <i>浮动</i> | 每个图案的Alpha与其比例相乘的强度。<br>注意：输出颜色是所有颜色乘数的加权结果。 |
| <b>形状索引乘数</b> <i>浮动</i> | 每个图案的Alpha与其归一化索引相乘的强度。<br>注意：输出颜色是所有颜色乘数的加权结果。 |
| <b>样条Height乘数</b> <i>浮动</i> | 每个图案的Alpha与其所在位置的样条Height相乘的强度。<br>注意：输出颜色是所有颜色乘数的加权结果。 |
| <b>随机明亮度</b> <i>浮动</i> | 将随机乘数应用于指定值，以减少图案的Alpha。<br>注意：输出颜色是所有颜色乘数的加权结果。 |
...
| <b>蒙版映射输入采样模式</b> <i>整数</i> | 将蒙版映射中的值映射到样条的方法： <br>*-纹理空间*：将这些值应用于样条，如果使用纹理的UV坐标将这些值放置在纹理中。 这有效地将值应用于样条的“原位”；<br>*— 沿样条水平*：值直接应用于编码后的样条坐标（请参阅样条坐标输入），其中每行从上到下应用于不同的样条；<br>*-Hor。 沿样条线(rand. 偏移X)*：值直接应用于已编码的样条坐标（请参阅样条坐标输入），并且在每个样条的“比例映射”中具有随机水平偏移（即，样条坐标中的每一行）；<br>*- Hor。 沿样条线(rand. 偏移Y)*：值直接应用于已编码的样条坐标（请参阅样条坐标输入），并且在每个样条的“比例映射”（即，样条坐标中的每一行）中具有随机垂直偏移。 |
