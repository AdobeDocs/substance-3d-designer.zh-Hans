---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/adjustments/color-match.html"
breadcrumb-title: ''
...
* **源颜色模式**： *平均，参数，输入*&#x200B;设置源颜色是通过平均输入图像、通过设置参数还是通过输入插槽来定义。
* **源颜色**： *（颜色值）*&#x200B;如果源颜色模式设置为*参数*，则此参数确定源颜色。
* **目标颜色模式**： *参数，图像输入*&#x200B;设置是否通过平均输入图像、设置参数或使用输入插槽来定义源颜色。
* **目标颜色**： *（颜色值）*&#x200B;如果“目标颜色模式”设置为*参数*，则此参数确定目标颜色。
* **自定义颜色变量**： False/True\
  启用其他颜色变化。
* **颜色变化**\
  在启用时为结果设置色相、色度或明亮度变化。
* **使用蒙版**： *False/True*\
  根据下面的“蒙版模式”，切换“蒙版输入”或“输出”的使用。
* **蒙版模式**： *参数，输入*&#x200B;参数模式输出一个蒙版，其中详细说明了颜色的更改方式。 输入模式允许蒙版控制色彩匹配效果的强度。
* **蒙版**\
