...
|  |  |
| --- | --- |
| <b>输入</b>颜色 | 基于蒙版的颜色应从中提取蒙版的输入彩色图像。 |
| <b>颜色输入</b>颜色&#x200B;*在“使用颜色输入”设置为“真”时可用* | 用于定义每个像素的参考颜色的输入颜色图像。 |
...
| <b>颜色</b>Float3 *当“使用颜色输入”设置为“False”时可用* | 应围绕其执行颜色选择的参考统一颜色。 |
...
| <b>距离粗细</b>浮点3 | Lab颜色距离算法(DeltaE2000)为每个亮度、色度和色相值引入一定的权重因子。   较低的值会减小色差算法中各种因素的影响。   由于眼睛通常接受比色度(C)或色相(H)更大的明度(L)差异，因此(L:C:H)的默认比率为(0.5:1:1)。 0.5:1:1比例将产生两倍于色度或色相的光度差异。 |
