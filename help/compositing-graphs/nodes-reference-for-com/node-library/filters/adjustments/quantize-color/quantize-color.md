...
| <b>ID排序模式</b> *整数* | 在&#x200B;*立方体*&#x200B;中排列颜色，其中宽度、Height和深度是渐变，其中颜色的每个分量从0增加到1(例如 红、绿、蓝RGB)。   此参数选择用于在提取的调色板中对颜色列表以及提取的ID映射区域中的索引进行排序的方法：<ul data-preserve-html="true"> <li data-preserve-html="true"><b>Z曲线：</b>颜色会根据Z曲线在颜色立方体中找到的下一个颜色进行排序（从白色到黑色）</li> <li data-preserve-html="true"><b>色相：</b>颜色按最接近的色相排序</li> <li data-preserve-html="true"><b>代表性：</b>颜色在量化图像中使用率从高到低排序</li> </ul> |
| <b>缩小筛选</b> *整数* | 该颜色量化过程涉及计算图像在减小尺寸（即缩放）下的直方图，以便按重要性对其颜色进行排序。 此参数控制在计算其直方图之前对缩小图像进行滤波的方法：<ul data-preserve-html="true"> <li data-preserve-html="true"><b>双线性：</b>将双线性滤波应用于图像，生成带有插值颜色的直方图，这些颜色可能不是原始图像的一部分，从而稀释了某些原始颜色。 这有助于处理使用多种颜色的图像。</li> <li data-preserve-html="true"><b>最接近：</b>对最接近像素的颜色进行采样，不进行过滤，从而生成只使用原始图像颜色的直方图。 这适合使用少量颜色的图像。</li> </ul> |
