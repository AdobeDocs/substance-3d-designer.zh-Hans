...
* **Height：** *灰度输入* Height是材料槽中唯一必需的输入。 没有它，节点将无法正常工作。
* **Emissive：***颜色输入* Emissive应采用以下格式：纯黑色不发光，任何其他彩色值都发光。 Alpha将被忽略。 需要连接到此插槽或环境插槽才能看到任何结果。
* **环境**： *颜色输入*\
  使用HDR光照环境计算辐照度。 需要连接到此插槽或Emissive插槽才能看到任何结果。
