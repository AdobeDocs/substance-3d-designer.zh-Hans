---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/channels/rgba-merge.html"
breadcrumb-title: ''
description: 使用“RGBA合并”节点将单独的红色、绿色、蓝色和Alpha通道组合到单个RGBA纹理中。
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Filters > Channels > RGBA Merge
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: RGBA合并
user-guide-description: ''
user-guide-title: ''
source-git-commit: 6c55ac0f1f6da5bc5683a34a4eca174f978eac64
workflow-type: tm+mt
source-wordcount: '109'
ht-degree: 2%

---


# RGBA合并

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

![](../../../../../../assets/rgba-merge.png){width="128px"}

## RGBA合并

**范围：** *筛选器/通道*

**简单**

</td>
<td style="border: 0;" valign="top">

## 描述

将单独的灰度输入打包到四个声道中的每一个声道中。 不要与“RGBA合并”混淆，因为此节点可提供更多的合并控制！

一起进行声道打包映射非常有用。 例如，可用于将Smoothness、金属和AO插入各自的R、G和B通道。

## 参数

### 输入

* **R**： *灰度输入*
* **G**： *灰度输入*
* **B**： *灰度输入*
* **A**： *灰度输入*

*无参数。*

## 示例图像

|  |
| --- |
| 没有附加到此页面的图像。 |

</td>
</tr>
</table>
