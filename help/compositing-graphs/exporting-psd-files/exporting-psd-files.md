...
Substance 3D Designer允许将纹理导出为Adobe Photoshop文档或PSD文件。本页说明了用于将图形的节点转换为层的特殊界面。**此过程不是自动的：您拥有很多控制权，但它是有限的，并且通常无法获得节点和图层之间的准确匹配。** 此外，无法保证您的PSD包含与图形相同的输出，除非您明确将其设置为这样做。 一般来说，您想要的越准确和正确，用户需要花费的精力就越多。 通常，唯一能够以非破坏性方式密切复制的内容是[混合节点](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/blend/blend.md)。 不支持调整图层，图层样式或图层混合模式之外的任何其他内容均不受支持。

[Substance 3D Designer也可以导出为位图文件。](../../compositing-graphs/exporting-bitmaps/exporting-bitmaps.md)

## PSD导出对话框

只能通过一种方法打开PSD导出对话框。 在要导出为PSD的图形的[图形视图](../../interface/the-graph-view/the-graph-view.md)中，单击![](../../assets/image2019-9-17-14-44-17.png) <b>工具</b>按钮，然后选择<b>PSD导出器</b>。 该界面在<b>图形视图</b>中变为可见。

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

![PSD导出器用户界面](../../assets/psd-dialog.png "PSD导出器用户界面")
...
1. <b>添加组：</b>添加图层组
1. <b>添加图层下拉列表：</b>从以下两种方法中选择一种来添加图层。 还可以通过&#x200B;*使用鼠标右键将节点拖到栈栈*&#x200B;来添加图层。
1. <b>移除图层下拉列表：</b>移除选定图层或所有图层。
1. <b>Layerstack：</b>如果在此处执行大多数设置工作。 界面镜像Photoshop中的有限选项。 在此处设置图层名称、混合模式和不透明度。 如果图层有两个缩览图，则第二个缩览图代表Alpha通道。
...
由于Photoshop不直接支持多输出素材，因此有多种方式可设置您的PSD。 以下是常用方法的简述。
