...
+++图形工具栏
在“图形视图”工具栏中，单击<b>节点调板</b>中的“框架”按钮。
...
如果在创建帧时图中的选区处于活动状态，则该帧将被自动调整以完全包括所选对象。
...
>[!TIP]
>
> 创建框架后，框架的“标题”属性会自动获得焦点，因此您可以立即编辑框架的标题。
...
默认情况下，框架在移动或调整大小时会对齐到中等网格。

按住<b>Ctrl</b> (Windows) / <b>Cmd</b> (macOS)键可将此对齐方式移动到小网格以进行更精细的调整。
...
+++颜色
<b>帧颜色</b>用于在“图形”视图中填充帧。 使用拾色器选择任意颜色。
...
框架可以用要置入框架内的文本进行批注。 文本与左侧对齐，并从框架的左上角开始。 使用框架的[描述](#properties)属性编辑该文本。
...
<b>标题</b>以粗体字体显示，位于框架的左上方。 可以打开或关闭字幕的可见性。

它的大小可以锁定在最小屏幕大小处，因此当缩小图表时，它保持可读性。 您可以通过选中[图形视图](../../../../interface/the-graph-view/the-graph-view.md)工具栏的<b>信息</b>下拉列表中的“框架标题”选项来执行此操作。
...
您可以在框架的“描述”属性中复制并粘贴此示例，以自行测试此功能：
...
如果某个对象满足其包含规则，则该对象将被视为包含在框架中。 这些规则因对象和特殊情况而异。 它们如下所示。

每个插图中的黄色符号表示一个点或区域，它们需要完全位于框架的边界内，以便对象包含在该框架中。
...
这意味着嵌套框架必须完全位于另一个框架的边界内才能包括在后一个框架中。
