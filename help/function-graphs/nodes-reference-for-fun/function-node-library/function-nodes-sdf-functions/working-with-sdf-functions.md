...
您需要了解有关Substance函数图的信息：
* 与Substance图类似，节点连接器是&#x200B;*专用的*，这意味着它们只能连接到&#x200B;*匹配颜色* [表示其类型](../../function-nodes-overview/function-nodes-overview.md#color-coding)的其他连接器。
...
该节点提供了用于可视化SDF 函数各方面的专用功能，这些功能将让我们更直观、更高效地构建节点，例如边界框架和等高线。
...
与Substance函数图中的所有节点一样，SDF 函数节点没有参数，只有作为参数的输入连接器。

若要设置这些输入的值，可使用[常量节点](../../atomic-function-nodes/constant-nodes/constant-nodes.md)，如&#x200B;**Float**、**Float3**&#x200B;和&#x200B;**Integer3**。\
您可以按照常规方式通过节点菜单创建这些项，也可以从连接器拖动新连接以从匹配类型节点的过滤列表中受益。
...
>[!TIP]
> 
> <table style="border: none"><tr style="border: none"><td style="border: none; vertical-align: top"><p>如果您不需要始终保持某些值可见，请使用<code>D</code>键停靠节点以节省空间并清空图表。</p><p>您还可以使用注释来跟踪值。</p></td><td style="border: none; width: 67%; vertical-align: top"><img src="./working-with-sdf-functions.resources/working-with-sdf-docked-nodes.png" alt="SDF 函数节点上输入连接器的工具提示。" /></td></tr></table>
...
<table style="border: none">
    <tr style="border: none">
        <td style="border: none; vertical-align: top">
            <p>边框是3D空间中的一个框，它定义了<i>边界</i>，在<a href="../../../../compositing-graphs/nodes-reference-for-com/node-library/texture-generators/patterns/shape-splatter-v2/shape-splatter-v2.md">形状飞溅v2</a>SDF 函数中计算和绘制该节点。</p><p>如果边框太小，可能会裁切部分形状。 如果它过大，则可能导致不必要的计算和较长的处理时间。</p><p>使用<b>定界帧</b>参数可以启用定界帧的可视化。 然后，您可以通过更改<b>定界帧大小</b>参数的值来调整定界帧的大小。</p><p>使用<b>帧外着色</b>参数可以将定界帧之外的区域显示为亮红色，以便您可以相应地调整帧。</p>
        </td>
...
<table style="border: none">
    <tr style="border: none">
        <td style="border: none; vertical-align: top">
            <p>因为变换形状涉及实际*变换绘制形状时的空间*，所以某些变换后使用的节点的结果可能令人惊讶。<br>在这种情况下，将空间本身可视化会很有帮助，可以通过<i>将形状的距离场</i>可视化来实现。</p><p>为此，3D查看器节点使用<i>等高线</i>，这些等高线是表示与形状表面相距给定距离的重复轮廓线。 <b>SDF等值线</b>参数启用该可视化。<br>等高线绘制在位于<b>SDF等高线位置</b>参数所指定Height的水平平面上。</p><p>观察等值线是如何通过应用于形状的变换而变形的，有助于了解形状本身的变换方式，并相应地调整节点的参数。</p>
        </td>
        <td style="border: none; width: 33%; vertical-align: top">
...
[胶囊](./sdf-functions-primitives/3d-sdf-capsule/3d-sdf-capsule.md)\
[圆锥](./sdf-functions-primitives/3d-sdf-cone/3d-sdf-cone.md)\
...
[地平面](./sdf-functions-primitives/3d-sdf-ground-plane/3d-sdf-ground-plane.md)\
...
基本材质管理适用于使用SDF 函数创作的形状。

您可以定义基本的材质属性：颜色、粗糙度和金属度，以用于3D查看器节点中的直接可视化，或作为“形状飞溅”v2节点中材质工作的基础。\
您也可以将材质ID指定给形状的不同部分以将其分开。
...
当我们对形状应用变换，例如偏移或旋转，我们实际上变换形状定义的空间。

如果我们想将变换应用到其他形状，例如，如果我们想以相同的方式旋转多个形状，我们需要确保它们都使用相同的变换空间。

转换的空间通过使用其专用的`P`输入在节点之间共享，您可以在大多数SDF节点中找到该输入。\
“P”表示世界空间&#x200B;**P**&#x200B;位置：表示世界空间中点坐标的3D矢量。

[偏移P](sdf-functions-transforms/3d-sdf-transform-offset-p/3d-sdf-transform-offset-p.md)和[旋转P](sdf-functions-transforms/3d-sdf-transform-rotate-p/3d-sdf-transform-rotate-p.md)节点会转换空间，并允许您将该转换传播到应继承它的所有节点。\
...
>[!TIP]
> 
> 要轻松重复使用SDF 函数，请将其复制到新的Substance函数图形，并在3D查看器和形状飞溅v2节点中将该图形用作&#x200B;**实例节点**。
> 
> 这提供了以下几项好处：
> * 您对函数所做的任何更新都将反映在两个节点中，而无需再次复制粘贴。 这是对复杂形状生活质量的极大改进。
> * 图形可具有在实例节点中可见的描述性名称，这将使使用您自己的SDF形状库更加易于管理，并使您的图形更具可读性。
> * 您可以为函数图表创建输入，该函数图表可用于[Get](../../atomic-function-nodes/get-nodes/get-nodes.md)节点。 这些输入将作为输入连接器显示在实例节点中，并允许您轻松进行形状变化。

### 材质 ID

可以为一个SDF形状分配一个材质ID，它是一个整数值，可用于区分形状的某些部分，并在[3D查看器](../../../../compositing-graphs/nodes-reference-for-com/node-library/filters/effects/3d-viewer/3d-viewer.md)和[形状飞溅v2](../../../../compositing-graphs/nodes-reference-for-com/node-library/texture-generators/patterns/shape-splatter-v2/shape-splatter-v2.md)节点中为它们分配不同的材质。

请注意，具有不同材质ID的曲面用混合形状上的硬边分割，如下例所示。

在要用特定素材ID标记的形状部分后面使用[设置素材ID](./sdf-functions-material/set-id/set-id.md)节点，并使用[整数](../../atomic-function-nodes/constant-nodes/constant-nodes.md)常量节点设置所需的素材ID值。\
在3D查看器节点中，将&#x200B;**输出**&#x200B;参数设置为`Material ID`以可视化形状的材质ID。

![working-with-sdf-material-id.png](working-with-sdf-functions.resources/working-with-sdf-material-id-01.png)\
*在右侧，合成两个3D查看器节点的输出以显示形状（左）及其材质ID（右），以说明在混合形状中，如何在材质ID拆分时插入材质。*

Shape spplater v2伴随节点可以利用“材质ID”：
* [形状飞溅v2映射器](../../../../compositing-graphs/nodes-reference-for-com/node-library/texture-generators/patterns/shape-splatter-v2-mapper-color/shape-splatter-v2-mapper-color.md)节点可以使用这些素材ID来分配不同的图案。
* [要遮盖的形状飞溅v2](../../../../compositing-graphs/nodes-reference-for-com/node-library/texture-generators/patterns/shape-splatter-v2-to-mask/shape-splatter-v2-to-mask.md)可以根据形状的材质ID遮盖部分形状。

<table style="border: none; margin-top: 32px">
    <tr style="border: 0">
        <td style="border: 0; width: 33%">
            <img src="./working-with-sdf-functions.resources/working-with-sdf-material-id-06.jpg" alt="用于在形状飞溅v2映射器颜色节点中进行颜色映射的SDF素材ID。"/><i>用于形状飞溅v2映射器颜色中的颜色映射的材质ID<br></i>
        </td>
        <td style="border: 0; width: 33%">
