...
您可以在[Substance函数图形](../../../function-graphs/function-graphs.md)中使用内置变量来访问特定值。 它们始终以`$`（美元）符号开头。
...
| $pixelration | 整数 | 返回与当前整数像素比率对应的节点值（继承或绝对）： 0：拉伸1：方形 |
| $拼贴 | 整数 | 返回与当前拼贴模式对应的节点值（继承或绝对）： 0：无拼贴1：水平拼贴2：垂直拼贴3：H和V拼贴 |
...
| $深度 | 浮点 | 返回[FX-Map](../../../compositing-graphs/nodes-reference-for-com/atomic-nodes/fx-map/fx-map.md)节点的八度音阶（级别）数。 这允许节点根据它所代表的四叉树中的级别来修改其行为。 |
| $depthpow2 | 浮点 | 如上所示，但返回2的乘反值，其值为八度音阶（音阶）数字的次方 — 即1/（2^八度音阶）。 这是一个助手值，对于某些常见计算很有用。 |
| $number | 浮点 | 返回绘制的模式的编号。 这可以通过控制[迭代](../../../function-graphs/fxmaps/using-functions-in-fxmaps/iterate-and-number-var/iterate-and-number-variable.md)图形的动态函数节点来访问，以便在每个迭代步骤修改其行为。   请注意，`$number`从0开始计数，而不是1。   在使用迭代节点链时，`$number`变量将返回使用函数参数之前连接的最后一个迭代节点的迭代号。 如果要从多个Iterate迭代中检索变量编号，则应使用“自定义变量”到[Set](../../../function-graphs/fxmaps/using-functions-in-fxmaps/using-the-set-sequence/using-the-set-sequence-nodes.md)节点。 |
