---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-designer/release-notes/old-versions/version-2020-2-10-2.html"
breadcrumb-title: ''
...
**Substance Designer2020.2**&#x200B;带来了新的Substance 引擎更新、改进了公开参数的工作流程、某些性能改进和新内容。
...
* **输入图像的默认值**\
  现在，图形中的图像输入可以定义默认颜色，以便在未加载任何内容或未连接到该输入内容时回退到适当的值。

  ![](../../../assets/input-color-default.jpg){width="600px"}

* **新的距离模式**\
  [距离节点](../../../compositing-graphs/nodes-reference-for-com/atomic-nodes/distance/distance.md)现在具有两种新模式，它们改变了计算的执行方式：

  * **欧几里德**（原始模式）：欧几里德几何中点之间的直线距离。
  * **曼哈顿**：点之间的距离，但限制在网格上，类似于城市街区。
  * **切比雪夫**：距离大小一致的网格上两点之差的最大距离。

  ![](../../../assets/distance-comparison.jpg){width="600px"}

* **新的渐变插值**&#x200B;[&#x200B;渐变节点](../../../compositing-graphs/nodes-reference-for-com/atomic-nodes/gradient-map/gradient-map.md)具有新的&#x200B;**平滑**&#x200B;模式，用于插值键，从而生成更自然的颜色转换。 它通过查看邻居键来工作。 上一个模式&#x200B;**Interpolation**&#x200B;已重命名为&#x200B;**平面正切**。

  ![](../../../assets/gradient-comparison.png){width="600px"}

  ![](../../../assets/gradient-dropdow.png)

  **平滑**&#x200B;模式还有另一个参数，用于控制用于计算颜色之间混合曲线的正切的长度。 值0表示正切的长度为0，而值或1表示正切将是两个键之间长度的一半。

  ![](../../../assets/gradient-smooth-param.png)

* **改进的曲线节点**&#x200B;[&#x200B;曲线节点](../../../compositing-graphs/nodes-reference-for-com/atomic-nodes/curve/curve.md)现在可以直接输出灰度图像。 此更改使得无需先插入渐变纹理即可定义曲线并直接输出它。
...
公开和编辑参数的工作流程经过改进，更加直观：

* **改进的菜单操作**&#x200B;公开和编辑公开参数的操作已分开，以更加明确。 新快捷键也已引入，可更轻松、更快速地编辑公开参数。 例如，现在有一个专用按钮用于编辑参数的函数，还有一个专用操作用于跳转到公开的图形参数。

  ![](../../../assets/param-edit.png)

  ![](../../../assets/param-expose.png)

* **直接从对话框中配置新参数**&#x200B;公开参数时，现在可以从&#x200B;**公开参数**&#x200B;对话框中访问所有常规控件，如说明和默认值。 这样可以更轻松、更快速地配置和创建新的图形参数，从而避免在创建参数后立即进入图形参数视图。
...
借助改进的界面，现在可以更轻松地在图形中设置缩览图：

* **自动图形缩略图生成**\
  现在，只需单击图形参数中图标旁边的“生成”按钮，即可自动为Substance图形生成预览。 它目前仅支持将金属/粗糙度PBR图形用作输出节点。 在生成缩略图时，位移强度根据定义的物理尺寸值计算，否则默认为0.1值。
...
此新版本中提供了两个新的优化功能，可减少迭代之间的时间：

* **改进了计算性能**\
  现在当请求时直接计算输出节点，而不是在途中计算中间节点（现在是在第二步中）。 此更改允许更快地预览图形的结果，即使在调整根节点时也是如此。 在下面的比较中，该优化允许您在同一时间段查看图形的更多迭代（这里的材料具有4K分辨率）：

  ![](../../../assets/perf-graph.gif)

* **改进了膨胀和漫射生成**\
  现在，生成纹理的膨胀和漫射的速度比以前的版本快4倍。 这减少了烘焙之间的等待时间。
