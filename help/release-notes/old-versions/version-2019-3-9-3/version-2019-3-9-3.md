---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-designer/release-notes/old-versions/version-2019-3-9-3.html"
breadcrumb-title: ''
...
**Substance Designer2019.3**&#x200B;引入了色彩管理，支持OpenColorIO、改进的烘焙器更新和新的Atlas Scatter节点。
...
Substance Designer现在支持色彩管理，它允许您控制如何解释和转换颜色。 此功能支持[OpenColorIO](https://opencolorio.org/)，后者是娱乐和VFX行业中广泛使用的色彩管理系统。

* **在主设置中设置色彩管理**\
  默认情况下，Substance Designer将设置为“旧版”模式，这意味着它不会加载任何特定配置，并将像以前一样继续工作。 例如，要切换到OpenColorIO，只需转到主要设置，然后在项目设置中配置并启用它。
...
* **色彩管理以3种不同的方式应用**\
  ...
  * **2D和3D视图**\
    实时opengl视口和Iray都可以进行色彩管理。 这允许在上下文中预览材质和纹理。 可通过专用下拉菜单轻松切换颜色配置文件。 还可以在线性灰度系数中显示视图以在没有颜色校正的情况下进行预览。

    ![](../../../assets/color-2d-view.png){width="400px"}

    ![](../../../assets/color-3d-view.png){width="400px"}
  * **图形输入和输出以及位图导出**\
    当在图形的入口和出口读取和写入位图图像时，色彩管理可以调整图像并将其转换为在适当条件下工作。 这可以在图形中的位图主首选项中控制，并在导出过程中在导出对话框中修改。 从2D视图中保存图像还将提供一种选择色彩空间的方法。
...
已从头重新创建网格烘焙器的曲率。 它现在利用光线追踪技术，不仅允许GPU加速，还增加了网格交叉点的支持。

有关详细信息，请参阅[网格曲率](https://experienceleague.adobe.com/zh-hans/docs/substance-3d/bakers/bakers-settings/curvature-from-mesh)文档。

>[!NOTE]
>
> 先前来自网格烘焙器的曲率现已弃用。 我们建议切换到新版本。

### 改进了网格烘焙器的环境遮蔽

![](../../../assets/banner-ao-v2.png)

“来自网格烘焙器的环境遮蔽”现在有一个名为“地平面”的新设置，它模拟网格下的平面以产生地面遮蔽。 这有助于为静态几何或非有机对象生成更有趣的外观效果。 默认情况下，“地平面”位于网格定界框的底部，但可通过专用设置进一步推动。

有关详细信息，请参阅[Mesh中的环境遮蔽](https://experienceleague.adobe.com/zh-hans/docs/substance-3d/bakers/bakers-settings/ambient-occlusion-from-mesh)文档。
...
* **更快的预览模式**\
  现在，编辑图形参数时切换到预览模式的速度更快了，特别是在具有许多公开参数的图形上。 预览模式现在是一个专用的选项卡，可简化参数编辑器与其预览之间的来回切换。
* **已修改预设编辑器**\
  现在，在Substance Designer中管理预设的方式更加简单快捷：预设编辑器现在有一个专用的选项卡，每个预设现在列出它们影响的参数。
* **可视条件现在考虑了2D变换小工具**\
  当变换2D Gizmo具有“可见条件”时，它现在可以在2D视图中隐藏，从而使其与参数相关联。

### 新内容

![](../../../assets/banner-content-8.jpg)

在此版本中，我们引入了新的&#x200B;**Atlas Scatter**&#x200B;节点。 该节点允许读取地图集图像（包含子图像的图像）并自动将其内容分离成可随机散布的单个分量。 例如，此节点可用于在Dirt地面材料上放置分支和树叶。 [Substance Source](https://source.substance3d.com/allassets?q=atlas)现在包括许多可以随此新节点散布的Atlas素材。
