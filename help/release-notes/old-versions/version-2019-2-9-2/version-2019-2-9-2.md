---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-designer/release-notes/old-versions/version-2019-2-9-2.html"
breadcrumb-title: ''
...
* **从节点中单击并拖动链接时打开菜单**\
  现在，单击并拖动节点连接并松开鼠标时，它将打开“节点创建”菜单。 随后创建的节点将连接到该链接。 列表中可用的节点将取决于传入连接。![](version-2019-2-9-2.resources/click-drag-insert-node.gif)
...
* **涂层**\
  涂层允许在基础材质上方形成第二层材料。 可用于模拟汽车绘画或金属表面的薄膜。\
  ...
  * **涂层粗糙度**：涂层的粗糙度。
  ...
  注意：涂层需要新的着色器“physically\_metallic\_roughness\_coated”。

  ![](version-2019-2-9-2.resources/coating-carbon-fiber-0.jpg){width="400px"}
* **次表面散射**\
  次表面散射模拟材料内部光线的吸收和漫射。 它对模拟皮肤和其它有机表面非常有用。\
  要创建子表面材料，您可以使用新Substance模板“PBR子表面散射(金属/粗糙度)”或将以下输出添加到图形：

  * **散射**：控制材料内光线漫射的强度。

  注意： Subsurface需要新的着色器“physically\_metallic\_roughness\_sss” 。 散射颜色可以通过着色器参数来控制。

  ![](version-2019-2-9-2.resources/sub-surface-scattering-0.jpg){width="400px"}

>[!NOTE]
>
> 所有这些新的渲染功能之所以成为可能，要归功于Substance Designer系统的重新设计。 我们的&#x200B;**GLSLFX**&#x200B;格式现在支持“**技术**”的定义，该定义允许多次渲染。 有关一些示例，请查看我们在&#x200B;**Substance Designer/资源/view3d/shaders**&#x200B;中提供的默认着色器

### 改进了性能

![](version-2019-2-9-2.resources/library-loading-optim.gif)

对引擎进行了一些改进（跳转到版本7.2.0），这在以下几种情况下是有益的：

* **缓存计算以加快微调**\
  通过添加新的缓存系统，改进了图形渲染。 这使得调整图形比以前快得多，因为我们现在跳过未更改的节点。
* **改进了库加载时间**\
...
在此新版本中，我们引入了许多专门用于处理环境图的新滤镜。

* **HDR合并**:Bring&#x200B;多张LDR（低动态范围）照片，创建一个HDR图像。
* **Nadir Patch**:This&#x200B;节点允许您在考虑球面变形的情况下克隆/修补HDRI的地面部分。
* **Nadir Extract**：此节点可以从HDRI中提取纹理。 它可用于映射3D 场景中的地面纹理。
* **Color Temperature Adjustment**：调整输入图像的色温，包括HDR 值的支持。
* **拉直水平线**：旋转环境图的经纬度。 根据拍摄条件，生成的360°全景图可能不是直的。
* **物理太阳和天空**：此节点是Hosek-Wilkie Skylight模型的实现。 指定太阳位置，它将生成物理上精确的天空全景图。
* **平面光**：生成类似于3D形状的矩形光。 可以更改颜色在2D 视图中的位置、应用纹理并使用色温或RGB值设置颜色。
* **线光**：生成线形光，并更改其在2D 视图中的位置。 除了专门为霓虹灯等细长形状设计的选项之外，它还有与平面光类似的选项。
* **球面光**：生成球面光，并使用2D 视图小工具更改其位置。 您还可以应用球面纹理来创建遥远的行星。
...
* [Iray]更新至NVIDIAIrayRTX 2019.1.3 SDK (317500.3714)
* [资源管理器]允许将任何类型的文件作为资源链接到包中
* [GradientNode]按ESC可取消渐变选取
* [参数]删除标识符上的自动大写
* [Project]添加一个选项，以指定默认情况下图形和资源是否在“库中可见”
* [预设]自动大头针修改后的参数
...
* [图形]图形显示中的链接Thickness不正确
* [图形]调整参数时触发的无效验证过多
* [图形]在打开包的两个窗口的情况下关闭包并使用上下文编辑时崩溃
* [函数图形]关闭函数视图时未显示警告
* [3D 视图]当3D 视图崩溃被设置为“正交”作为默认相机状态时，场景初始化
* [3D 视图]后FX DOF在Iray中保持启用状态
* [2D 视图]更改画笔大小时画笔选区窗口消失
* [2D 视图]信息面板：使用特定布局裁切值
* [2D 视图]最小化并恢复主窗口时，图像发生偏移
* [Baker] “来自资源”选择列表未正确筛选
* [Baker]在Embree上链接“来自网格的色图”和“来自网格的法线图”Baker时出现崩溃
* [Baker]烘焙每个顶点的弯曲会导致严重的伪影
* [资源管理器]无法导入UDIM资源并将其拖放到资源管理器中
* [资源管理器]链接异常文件资源管理器后链接网格和字体时，字体窗口未正确筛选
* [资源管理器]当图形将“在库中显示”设置为“否”时，资源可见
* [内容]“电源”和“夹紧”输入顺序不正确
* [内容]未标记“RGBA合并”节点输入
* [Cooker]数值的连接仍然无效
...
* [库]在库中显示MDL文件
* [Parameters]公开参数时崩溃
* [Parameters]在下拉列表中重新创建新元素后崩溃
* [导出] 8K批量导出失败
* [预设]在SBS实例中应用涉及布尔运算的预设时崩溃
