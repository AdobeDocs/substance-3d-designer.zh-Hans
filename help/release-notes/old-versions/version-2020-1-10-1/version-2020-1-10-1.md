...
**Substance Designer2020.1**&#x200B;包括新的快捷方式编辑器、MaterialX增效工具以及通过Adobe 颜色引擎和新的烘焙器功能对色彩管理的许多新改进。
...
在此版本中，我们引入了一个新的快捷键编辑器，该编辑器允许定义键盘快捷键以加快不同图表模式中节点的创建。

新的快捷键编辑器可通过&#x200B;**编辑>首选项>快捷键**&#x200B;在主首选项中访问。 默认情况下，每个快捷键字段都为空。

* **每种图表类型的快捷键**\
  可以为Substance Designer中可用的每种类型的图形（从常规合成图形到更高级的图形，如函数和MDL）定义快捷方式。

  ![](../../../assets/shortcuts-graph-type.png)
* **为原子节点和自定义筛选器分配快捷键**\
  默认情况下，界面显示每种图形类型的原子节点。 使用搜索字段可以细化列表，还可以使自定义库筛选器和节点可访问：

  ![](../../../assets/shortcut-node-search.gif)
* **正在创建新快捷方式**\
  要为特定节点创建新的快捷键，只需单击其名称旁边的文本字段，然后在键盘上键入请求的键即可。 要清除快捷键，请使用叉号按钮。
...
在SIGGRAPH 2019期间，我们演示了用于Substance Designer的MaterialX增效工具。 [MaterialX](http://www.materialx.org/)是由[工业光和魔法(ILM)](https://www.ilm.com/)开发的格式，使艺术家能够创作材料，然后将其用于各种平台和多种用途（例如电影、电视节目、游戏、VR体验和制造业）。 此新增效工具现已在Beta版中提供，允许创建MaterialX素材，以便在其他兼容的DCC应用程序中使用。 请查看我们最近关于MaterialX[&#128279;](https://magazine.substance3d.com/research-a-portable-shader-graph/)的文章，以了解有关此新增效工具背后的过程和目标的更多信息。

此新插件允许：

* 并行开发程序着色器和纹理。
* 使用可在运行时更改的功能（如细节图和程序蒙版）为Substance Painter创建高级着色器。
...
除了OpenColorIO之外，现在还可以使用Adobe 颜色引擎(ACE)来支持ICC配置文件进行色彩管理。 这意味着您现在可以导入和导出校准图像，以匹配其他软件，如Photoshop和Illustrator。
...
* **已改进的取样**\
  环境遮蔽、Thickness和曲率烘焙机现在使用了一种新的采样方法，极大地提高了烘焙结果的质量。 它产生更吸引人的噪音和更稳定的结果，而无需增加样本数。 下图显示了上图中的上一次采样方法，下图中的新方法，从左到右4次、8次和16次采样（所有渲染均在消除锯齿设置为4x4次采样的情况下完成）。
...
* **新的未发光着色器**\
  默认情况下包括名为“unlit”的新着色器。 它不会对任何光照做出反应，而是可用于在视区中通过网格显示单个纹理。 例如，对于烘焙纹理而言，这是非常有用的。
