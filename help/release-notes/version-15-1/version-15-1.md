...
Substance Designer15.1提供了一个完全改版的图形创建窗口，其中包含直接样本访问、改进的噪声节点以实现更大的创意可能性、节点菜单中整理好的类别等等。
...
每个图形都经过<b>批注</b>，组织严谨，并包含最少的节点，使其尽可能易于理解。

您可以在创建新图形时访问“材料样本”类别中的样本，也可以使用方便的“转到样本”按钮直接从主屏幕访问样本。
...
噪声在大多数图形中起着至关重要的作用，因此我们重点关注此版本中的几项关键增强功能，以改进其功能和可用性。

在此更新中，我们引入了<b>对非拼贴方案的更好支持</b>，从而确保噪声模式按预期运行，无需强制拼贴。 以前，噪声节点在禁用拼贴时被强制平铺或生成错误的结果。

大多数噪声现在包含<b>新参数</b>，为用户提供了更好的创意控制。 这些附加选项使图形作者能够在其工作流程中微调噪声的外观和行为。

最后，位深度<b>不再硬锁定到16位</b>。 您现在可以覆盖单个节点实例上的位深度设置，以便在需要时获得更高的细节和动态范围，或者优化图形以提高性能。
...
当一个节点有多个[输出](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/output/output.md)时，不可能同时在2D 视图中显示所有这些输出或将其用作节点缩略图。 在这种情况下，主流准则是使用第一个连接的大头针，或者，如果没有连接，则默认使用第一个输出。

但是，这种方法不一定能产生最佳结果。 例如，在某些Spline节点中，第一个连接的大头针通常表示样条坐标数据，这不适合预览。
...
使用函数图形时，您可能需要确定图形中是否存在[变量](../../function-graphs/nodes-reference-for-fun/atomic-function-nodes/get-nodes/get-nodes.md)。
...
* [NewGraph]重工新的图形窗口
* [NewGraph]添加材料示例和高级示例
* [NewGraph]为模板数据（类别和副标题）添加新的图形属性
* [NewGraph]删除输出格式选项
* [内容]添加哈希函数
* [Content]向函数添加调色板。sbs
* [内容]各向异性噪声v2：添加默认输出格式，添加无序
* [内容]对节点和参数标签应用句首大写
* [内容] BnW污点1 v2：添加默认输出格式，不支持拼贴
* [内容] BnW污点2 v2：添加默认输出格式，不支持拼贴
* [内容] BnW污点3 v2：添加默认输出格式，不支持拼贴
* [内容]细胞1、2、3、4 v2：添加默认输出格式，不支持拼贴，无序选项
...
* [Baker]删除Baker时选择最接近的Baker
* [Baker]UDIM：定义要烘焙的UV磁贴列表
* [Baker]将烘焙sdk更新到3.15.4
* [3D 视图/SceneBrowser]右键单击基本美元时，应避免选择基本美元
* [色彩管理]支持ACE 2.0
* [合成图形]允许将输出节点设置为“默认输出”
* [Cooker]移除有关函数实例的非连接输入的警告†
* [函数] Add isDefined运算符
* [图形]按“组”属性在node菜单中分组项目
* [图形]改进缩览图渲染

### 修复

* [3D 视图] L16灰度纹理在连接到环境或baseColor时显示红色色调
* [3D 视图]更改没有材料的场景的材料绑定将创建一个新的“默认”材料
* [3D 视图]特定OBJ网格的计算法线不正确
* [3D 视图]在Pathtracer中加载时，SBSSCN中的自定义环境不可见
* [3D 视图]旋转已禁用的环境时，控制台出现错误
...
* [Baker]更新“color-generator”参数不适用于“灰度”
* [内容]路径蒙版：非方形比例失败
