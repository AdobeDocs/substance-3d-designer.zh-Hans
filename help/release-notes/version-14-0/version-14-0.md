...
* <b>渐变映射</b>：使用鼠标中键向左或向右移动所有[渐变键](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/gradient-map/gradient-map.md)（以便保留所有键之间的所有间隙）。
* <b>参数</b>：为了通过参数插入自定义函数，现在可以使用“编辑”函数构件。 这是使用[Substance函数图表](../../function-graphs/the-function-graph/the-function-graph.md)创建自定义工具以便驱动参数的强大解决方案。
